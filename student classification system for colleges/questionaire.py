...
def submit1():
    ...
    myfile=csv.writer(myfile, quoting=csv.QUOTE_ALL)
    myfile.writerow(res)
    print(res)
...
frame = Frame(canvas,width=1150,height=700)
canvas.create_window((0,0), window=frame, anchor='nw')
#frame.pack(fill = BOTH)
var = IntVar()
fo = open("questions.txt", "r+")
...
def prin(obj):
    ...
    Radiobutton(frame, text="Strongly disagree", variable=obj.var, value=0,command=obj.sel).pack( anchor = W )
    Radiobutton(frame, text="Disagree", variable=obj.var, value=1,command=obj.sel).pack( anchor = W )
    Radiobutton(frame, text="Neutral", variable=obj.var, value=2,command=obj.sel).pack( anchor = W )
    Radiobutton(frame, text="Agree", variable=obj.var, value=3,command=obj.sel).pack( anchor = W )
    Radiobutton(frame, text="Strongly agree", variable=obj.var, value=4,command=obj.sel).pack( anchor = W )
